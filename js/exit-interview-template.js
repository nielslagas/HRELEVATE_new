...
function saveTemplateToLocalStorage() {
    ...
    // Save checkboxes in the checkbox-list
    ...
    localStorage.setItem('exit-interview-template', JSON.stringify(data));
}

// Function to save the template data to local storage with confirmation
function saveTemplate() {
    saveTemplateToLocalStorage();
    alert('Je exit interview template is opgeslagen in je browser. Je kunt later terugkomen om verder te gaan.');
}

// Function to load the template data from local storage
function loadTemplate() {
    const savedData = localStorage.getItem('exit-interview-template');
    ...
    if (savedData) {
        ...
        // Restore checkboxes in the checkbox-list
        if (data.checkboxes) {
        ...
        alert('Je opgeslagen exit interview template is geladen.');
    } else {
        alert('Er is geen opgeslagen exit interview template gevonden.');
    }
}
...
function resetTemplate() {
    if (confirm('Weet je zeker dat je de template wilt resetten? Alle ingevulde gegevens worden gewist.')) {
        ...
        localStorage.removeItem('exit-interview-template');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (localStorage.getItem('exit-interview-template')) {
        if (confirm('Er is een opgeslagen exit interview template gevonden. Wil je deze laden?')) {
            loadTemplate();
        }
    }
});
